...
func testProjectToManyProjectAssociations(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadProjectAssociations(ctx, tx, false, (*[]*Project)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testProjectToManyTaskStatuses(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := ProjectSlice{&a}
	if err = a.L.LoadTaskStatuses(ctx, tx, false, (*[]*Project)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testProjectToManyAddOpProjectAssociations(t *testing.T) {
	var err error
...
func testProjectToManyAddOpTaskStatuses(t *testing.T) {
	var err error
...
var (
	projectDBTypes = map[string]string{`ID`: `uuid`, `Name`: `character varying`, `Abbreviation`: `character varying`, `Description`: `character varying`, `Active`: `boolean`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
