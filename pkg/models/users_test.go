...
func testUserToManyProjectAssociations(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, projectAssociationDBTypes, false, projectAssociationColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, projectAssociationDBTypes, false, projectAssociationColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.ProjectAssociations().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadProjectAssociations(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProjectAssociations); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ProjectAssociations = nil
	if err = a.L.LoadProjectAssociations(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProjectAssociations); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAssigneeTasks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.AssigneeID, a.ID)
	queries.Assign(&c.AssigneeID, a.ID)
	if err = b.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}
	if err = c.Insert(ctx, tx, boil.Infer()); err != nil {
	...
	check, err := a.AssigneeTasks().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if queries.Equal(v.AssigneeID, b.AssigneeID) {
			bFound = true
		}
		if queries.Equal(v.AssigneeID, c.AssigneeID) {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadAssigneeTasks(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AssigneeTasks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.AssigneeTasks = nil
	if err = a.L.LoadAssigneeTasks(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AssigneeTasks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyWorkLogs(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadWorkLogs(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpProjectAssociations(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e ProjectAssociation

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*ProjectAssociation{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, projectAssociationDBTypes, false, strmangle.SetComplement(projectAssociationPrimaryKeyColumns, projectAssociationColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*ProjectAssociation{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddProjectAssociations(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ProjectAssociations[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ProjectAssociations[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ProjectAssociations().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManyAddOpAssigneeTasks(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddAssigneeTasks(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.ID, first.AssigneeID) {
			t.Error("foreign key was wrong value", a.ID, first.AssigneeID)
		}
		if !queries.Equal(a.ID, second.AssigneeID) {
			t.Error("foreign key was wrong value", a.ID, second.AssigneeID)
		}

		if first.R.Assignee != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.Assignee != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.AssigneeTasks[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.AssigneeTasks[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.AssigneeTasks().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManySetOpAssigneeTasks(t *testing.T) {
	var err error
	...
	err = a.SetAssigneeTasks(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetAssigneeTasks(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.AssigneeID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.AssigneeID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.AssigneeID) {
		t.Error("foreign key was wrong value", a.ID, d.AssigneeID)
	}
	if !queries.Equal(a.ID, e.AssigneeID) {
		t.Error("foreign key was wrong value", a.ID, e.AssigneeID)
	}
	...
	if a.R.AssigneeTasks[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.AssigneeTasks[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpAssigneeTasks(t *testing.T) {
	var err error
	...
	err = a.AddAssigneeTasks(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveAssigneeTasks(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.AssigneeTasks) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.AssigneeTasks[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.AssigneeTasks[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}

func testUserToManyAddOpWorkLogs(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `Name`: `character varying`, `Email`: `character varying`, `AccessToken`: `character varying`, `RefeshToken`: `character varying`, `Active`: `boolean`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
