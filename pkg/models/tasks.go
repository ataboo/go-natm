...
func (taskL) LoadAssignee(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTask interface{}, mods queries.Applicator) error {
	var slice []*Task
	var object *Task
	...
	if singular {
		...
		if !queries.IsNil(object.AssigneeID) {
			args = append(args, object.AssigneeID)
		}
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.AssigneeID) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.AssigneeID) {
				args = append(args, obj.AssigneeID)
			}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.AssigneeID, foreign.ID) {
...
// LoadTaskStatus allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (taskL) LoadTaskStatus(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTask interface{}, mods queries.Applicator) error {
	var slice []*Task
	var object *Task
	...
	if singular {
		...
		args = append(args, object.TaskStatusID)
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if a == obj.TaskStatusID {
					continue Outer
				}
			}

			args = append(args, obj.TaskStatusID)
	...
	query := NewQuery(
		...
		qm.WhereIn(`task_statuses.id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for task_statuses")
	}
	if err = results.Err(); err != nil {
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.TaskStatusID == foreign.ID {
				local.R.TaskStatus = foreign
				if foreign.R == nil {
					foreign.R = &taskStatusR{}
				}
...
// RemoveAssignee relationship.
// Sets o.R.Assignee to nil.
...
// SetTaskStatus of the task to the related item.
// Sets o.R.TaskStatus to related.
