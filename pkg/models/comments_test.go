...
func testCommentToOneTaskUsingTask(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadTask(ctx, tx, false, (*[]*Comment)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Task == nil {
...
func testCommentToOneSetOpTaskUsingTask(t *testing.T) {
	var err error
...
var (
	commentDBTypes = map[string]string{`ID`: `uuid`, `TaskID`: `uuid`, `UserID`: `uuid`, `Message`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
