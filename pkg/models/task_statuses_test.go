...
func testTaskStatusToManyTasks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	for _, v := range check {
		if v.TaskStatusID == b.TaskStatusID {
			bFound = true
		}
		if v.TaskStatusID == c.TaskStatusID {
			cFound = true
		}
	}
	...
	if err = a.L.LoadTasks(ctx, tx, false, (*[]*TaskStatus)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTaskStatusToManyAddOpTasks(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.ID != first.TaskStatusID {
			t.Error("foreign key was wrong value", a.ID, first.TaskStatusID)
		}
		if a.ID != second.TaskStatusID {
			t.Error("foreign key was wrong value", a.ID, second.TaskStatusID)
		}

		if first.R.TaskStatus != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.TaskStatus != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
...
func testTaskStatusToOneProjectUsingProject(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadProject(ctx, tx, false, (*[]*TaskStatus)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTaskStatusToOneSetOpProjectUsingProject(t *testing.T) {
	var err error
...
var (
	taskStatusDBTypes = map[string]string{`ID`: `uuid`, `ProjectID`: `uuid`, `Name`: `character varying`, `Ordinal`: `integer`, `Active`: `boolean`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_                 = bytes.MinRead
)
